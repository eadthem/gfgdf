...
wipeDisk()
{
	...
	#hdparm --user-master u --security-set-pass erase /dev/$1
	#hdparm --user-master u --security-erase erase /dev/$1 >> /tmp/diskOut$1
	#hdparm --user-master u --security-unlock erase /dev/$1
	#hdparm --user-master u --security-disable erase /dev/$1
	#rm /tmp/diskOut$1
	...
	smartctl -i /dev/$1 -i | grep -i "Device Model\|Product" | tr -d '\n' >> "/tmp/diskLabel$1"
	smartctl -i /dev/$1 -i | grep -i "Serial Number" >> "/tmp/diskLabel$1"
